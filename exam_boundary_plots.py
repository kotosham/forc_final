...
def plot_results(times: np.ndarray, positions: np.ndarray, control_inputs: np.ndarray):
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('logs/plots/exam_boundary_errors.png')
    plt.close()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('logs/plots/exam_boundary_controls.png')
    plt.close()
...
def main():
    ...
    sim = Simulator(
        ...
        show_viewer=True,
        video_path="logs/videos/exam_boundary_plots.mp4",
        width=1920,
        height=1080
    )
